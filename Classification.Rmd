...
feature_names = c("age", "age_o", "go_out", "sports", "tvsports", "exercise", "dining", "museums", "art", "hiking","gaming", "clubbing", "reading", "tv", "theater", "movies", "concerts", "music", "shopping", "yoga", 
 ...
 "reading_o", "tv_o", "theater_o", "movies_o", "concerts_o", "music_o", "shopping_o", "yoga_o", "exphappy_o")
...
l1_net <- cv.glmnet(x, y, family="binomial", alpha=1)
...
# Kasia's feature
SD <- cbind(x, iid = SD[,'iid'], attr3_1 = SD[,'attr3_1'], match = SD[,'match'])
SD <- as.data.frame(SD)
...
cols <- c(colnames(x), 'iid', 'match'); length(cols); dim(data)
data <- SD[, cols]
data <- left_join(data, df, by = 'iid') %>%
  mutate(age_gap = abs(age - age_o)) %>%
  subset(select = c(-age, -age_o, -iid)) %>% 
  mutate(samerace = as.factor(samerace),
