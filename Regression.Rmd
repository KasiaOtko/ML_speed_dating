...
nb_folds = 30
folds = vfold_cv(transformed_data, v = nb_folds)
# ann
error_per_fold = matrix(0L, nrow = nb_folds, ncol = 1)
nb_hidden_units = 8
for (i in 1:nb_folds) {
  # divides in outer train/test
  fold_data = folds$splits[[i]]$data
  ...
  error = ann(train,
              ...
              nb_hidden_units)
  # inner error for each inner fold
  error_per_fold[i, 1] = error[[2]]


}
ann_error_per_fold_ttest = error_per_fold
ann_gen_error_ttest = mean(error_per_fold)
...
base_gen_error_ttest = mean(error_per_fold)
...
l2_gen_error_ttest = mean(error_per_fold)
...
ttest_ann_l2 = t.test(z, alternative = "two.sided", alpha=0.05)
