...
for (i in 1:outer_nb_folds) {
  ...
  # calculates avg error for each model and trains once more
...
l2_gen_error = mean(error_per_fold)
l2_error_per_fold = error_per_fold
l2_params = params
l2_best_hyperparam_per_fold = best_model_hyperparam

## final param selection
weights = SampleWeight(y, weights_per_class)
params_to_use = l2_params[unique(l2_best_hyperparam_per_fold)]
l2_net <- cv.glmnet(data.matrix(x), data.matrix(y), family="binomial", alpha=0, lambda = params_to_use, weights = weights)
